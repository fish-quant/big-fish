...
def decompose_dense(
    ...
    if beta < 0:
        raise ValueError(
            "'beta' should be a positive value, not {0}".format(beta)
        )
    if gamma < 0:
        raise ValueError(
            "'gamma' should be a positive value, not {0}".format(gamma)
        )
    ...
    if kernel_size is not None:
        if isinstance(kernel_size, (tuple, list)):
            if len(kernel_size) != ndim:
                raise ValueError(
                    "'kernel_size' must be a scalar or a "
                    "sequence with {0} elements.".format(ndim)
                )
        else:
    ...
    precomputed_gaussian = precompute_erf(
        ndim=ndim, voxel_size=voxel_size, sigma=sigma, max_grid=max_grid
    )
    ...
    if len(spots_out_regions) + len(spots_in_regions) < len(spots):
        warnings.warn(
            "Problem occurs during the decomposition of dense "
    ...
    spots = np.concatenate(
        (spots_out_regions, spots_in_regions[:, :ndim]), axis=0
    )
...
def simulate_gaussian_mixture(
    ...
    if background < 0:
        raise ValueError(
            "Background value can't be negative: {0}".format(background)
        )
...
def _gaussian_mixture_3d(
    ...
    if 1 < nb_gaussian < limit_gaussian:
        ...
        warnings.warn(
            "Problem occurs during the decomposition of a dense "
