...
def test_rescale(dtype):
    ...
    if dtype in [np.uint8, np.uint16, np.uint32, np.int8, np.int16, np.int32]:
        ...
        min_, max_ = 0, 1
    assert tensor_rescaled.min() == min_
    assert tensor_rescaled.max() == max_
    assert tensor_rescaled.dtype == dtype
    assert tensor_rescaled.shape == (5, 5)
...
def test_stretching():
    ...
    tensor_expected = np.array([[0, 0, 0],
                                [65535, 65535, 65535],
                                [65535, 65535, 65535]], dtype=np.uint16)
    assert_array_equal(tensor_rescaled, tensor_expected)
